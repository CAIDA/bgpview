...
static int req_list_find(bgpview_io_zmq_client_broker_t *broker,
                         seq_num_t seq_num)
{
  int i;
...
static int server_subscribe(bgpview_io_zmq_client_broker_t *broker)
{
  ...
  if (zsocket_connect(broker->server_sub_socket, "%s", CFG->server_sub_uri) <
      0) {
...
static int handle_reply(bgpview_io_zmq_client_broker_t *broker)
{
  ...
  if (zsocket_rcvmore(broker->server_socket) == 0) {
    fprintf(stderr, "Invalid message received from server (missing seq num)\n");
    goto err;
  }
...
static int send_request(bgpview_io_zmq_client_broker_t *broker,
                        bgpview_io_zmq_client_broker_req_t *req, uint64_t clock)
{
  int i = 1;
  zmq_msg_t llm_cpy;
  int mask;
  ...
  /* send the seq num */
  if (zmq_send(broker->server_socket, &req->seq_num, sizeof(seq_num_t),
...
static int is_shutdown_time(bgpview_io_zmq_client_broker_t *broker,
                            uint64_t clock)
{
  if (broker->shutdown_time > 0 &&
      ((broker->req_count == 0) || (broker->shutdown_time <= clock))) {
    /* time to end */
    return 1;
  }
  return 0;
...
static int handle_timeouts(bgpview_io_zmq_client_broker_t *broker,
                           uint64_t clock)
{
  int idx;
  bgpview_io_zmq_client_broker_req_t *req;
  ...
  for (idx = 0; idx < MAX_OUTSTANDING_REQ; idx++) {
    ...
    if (--req->retries_remaining == 0) {
      ...
      fprintf(stderr, "DEBUG: Request %" PRIu32 " expired without reply, "
                      "abandoning\n",
              req->seq_num);
...
static int handle_server_msg(zloop_t *loop, zsock_t *reader, void *arg)
{
  ...
  while (retries < BGPVIEW_IO_ZMQ_CLIENT_BROKER_GREEDY_MAX_MSG) {
    ...
    if (broker->master_removed != 0 &&
      ...
      if (zloop_reader(broker->loop, broker->master_pipe, handle_master_msg,
                       broker) != 0) {
...
static int handle_signal_msg(zloop_t *loop, zsock_t *reader, void *arg)
{
  ...
  if (strcmp(command, "$TERM") == 0) {
    ...
    if (broker->shutdown_time == 0) {
      fprintf(stderr, "INFO: Got $TERM, shutting down client broker on next "
                      "cycle\n");
...
static int init_reactor(bgpview_io_zmq_client_broker_t *broker)
{
  ...
  if ((broker->timer_id = zloop_timer(broker->loop, CFG->heartbeat_interval, 0,
  ...
  if (zloop_reader(broker->loop, broker->signal_pipe, handle_signal_msg,
                   broker) != 0) {
...
static bgpview_io_zmq_client_broker_t *
broker_init(zsock_t *signal_pipe, bgpview_io_zmq_client_broker_config_t *cfg)
{
  bgpview_io_zmq_client_broker_t *broker;
