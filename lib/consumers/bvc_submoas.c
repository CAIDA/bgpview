...
/* our 'class' */
static bvc_t bvc_submoas = {
  BVC_ID_SUBMOAS,
  NAME,
  BVC_GENERATE_PTRS(submoas)
};
...
typedef struct subprefix_info {
  bgpstream_pfx_storage_t  superprefix;
  uint32_t subasn;
  uint32_t superasn;
  uint32_t prev_start;
  uint32_t start;
...
{
  bgpstream_pfx_storage_t subprefix;
  bgpstream_pfx_storage_t superprefix;
...
KHASH_INIT(subprefixes_in_superprefix,
...
KHASH_INIT(superprefix_map,
           bgpstream_pfx_storage_t,
           subprefixes_in_superprefix_t* , 1,
           bgpstream_pfx_storage_hash_val,
           bgpstream_pfx_storage_equal_val);
...
static int output_timeseries(bvc_t *consumer, uint32_t ts)
{
  ...
  timeseries_kp_set(state->kp, state->finished_submoas_pfxs_count_idx, state->finished_submoas_pfxs_count);
  timeseries_kp_set(state->kp, state->ongoing_submoas_pfxs_count_idx, state->ongoing_submoas_pfxs_count);
  timeseries_kp_set(state->kp, state->new_submoas_pfxs_count_idx, state->new_submoas_pfxs_count);
  timeseries_kp_set(state->kp, state->newrec_submoas_pfxs_count_idx, state->newrec_submoas_pfxs_count);
...
static int create_ts_metrics(bvc_t *consumer)
{
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "new_submoas_pfxs_count");
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "ongoing_submoas_pfxs_count");
  if((state->ongoing_submoas_pfxs_count_idx =
    ...
    snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "finished_submoas_pfxs_count");
...
static submoas_prefix_t add_new_asn(submoas_prefix_t submoas_struct, int asn, bgpstream_pfx_storage_t pfx, bgpstream_pfx_storage_t parent_pfx,uint32_t time_now){
  ...
  bgpstream_pfx_storage_t subp=pfx;
...
static char* print_submoas_info(int caller,bvc_t* consumer, bgpstream_pfx_t* parent_pfx, bgpstream_pfx_t* pfx, char *buffer, const int buffer_len ){
  ...
  buffer[0] = '\0';
  written = 0;
  bgpstream_patricia_node_t*  pfx_node = bgpstream_patricia_tree_search_exact(state->pt, parent_pfx);
...
//void update_patricia(bvc_t *consumer, bgpstream_patricia_node_t* pfx_node,pref_info_t *pref_info, uint32_t timenow){
  ...
  bgpstream_patricia_tree_result_set_t *res_set=bgpstream_patricia_tree_result_set_create();
  int same_origin=0;
    ...
    return;
      ...
      if(wandio_printf(state->file,"\n") == -1)
      {
        ...
        if (submoas_struct.number_of_subasns==0){
          ...
          if(wandio_printf(state->file,"%"PRIu32"|%s|%s|FINISHED|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s    \n",
                                           timenow,
                                           bgpstream_pfx_snprintf(pfx_str, INET6_ADDRSTRLEN+3, parent_pfx),
                                           bgpstream_pfx_snprintf(pfx2_str, INET6_ADDRSTRLEN+3, pfx),
                                           submoas_struct.first_seen,
                                           submoas_struct.start,
                                            timenow,
                                           print_submoas_info(2,consumer,parent_pfx, pfx, asn_buffer, MAX_BUFFER_LEN)) == -1)
                           {
                             fprintf(stderr, "ERROR: Could not write %s file\n",state->filename);
                             return ;
                           }
...
static void check_remove_submoas_asn(bvc_t *consumer,bgpstream_pfx_t *pfx, uint32_t asn){
  ...
  int i;
   for (i=0;i<submoas_struct.number_of_subasns;i++){
    ...
    if(k!=kh_end(state->superprefix_map)){
      ...
      i--;
      submoas_struct.number_of_subasns--;
      /* Check whether removing ASN from subprefix results in submoas being finished or not */
      if(check_submoas_over(consumer,submoas_struct, superprefix) || submoas_struct.number_of_subasns==0){
      ...
      if(wandio_printf(state->file,"%"PRIu32"|%s|%s|FINISHED|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s    \n",
                                                 timenow,
                                                 bgpstream_pfx_snprintf(pfx_str, INET6_ADDRSTRLEN+3, (bgpstream_pfx_t*)&superprefix),
                                                 bgpstream_pfx_snprintf(pfx2_str, INET6_ADDRSTRLEN+3, (bgpstream_pfx_t*)&submoas_struct.subprefix),
                                                 first_seen_ts,
                                                 submoas_struct.start,
                                                 submoas_struct.end,
                                                 print_submoas_info(4,consumer,(bgpstream_pfx_t*)&superprefix, (bgpstream_pfx_t*)&submoas_struct.subprefix, asn_buffer, MAX_BUFFER_LEN)) == -1)
                                 {
                                   fprintf(stderr, "ERROR: Could not write %s file\n",state->filename);
                                   return ;
                                 }
  ...
  if(submoas_struct.number_of_subasns>0){
    ...
    if(wandio_printf(state->file,"%"PRIu32"|%s|%s|%s|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s",
                                               ...
                                               submoas_struct.start,
                                               submoas_struct.end,
                                               print_submoas_info(5,consumer,(bgpstream_pfx_t*)&submoas_struct.superprefix, (bgpstream_pfx_t*)&submoas_struct.subprefix, asn_buffer, MAX_BUFFER_LEN)) == -1)
                               {
                                 fprintf(stderr, "ERROR: Could not write %s file\n",state->filename);
                                 return ;
                               }
...
void diag_check(bvc_t* consumer){
  ...
  khint_t k;
   for (k = kh_begin(state->subprefix_map); k != kh_end(state->subprefix_map); ++k) {
      if (kh_exist(state->subprefix_map,k)){
        bgpstream_pfx_snprintf(pfx_str, INET6_ADDRSTRLEN+3, (bgpstream_pfx_t*)&kh_key(state->subprefix_map,k));
        bgpstream_pfx_snprintf(pfx2_str, INET6_ADDRSTRLEN+3, (bgpstream_pfx_t*)&kh_value(state->subprefix_map,k).superprefix);
...
static void check_remove_superprefix(bvc_t* consumer, bgpstream_pfx_t* pfx ){
  ...
  int is_subprefix=0;
  j=kh_get(subprefix_map, state->subprefix_map, *(bgpstream_pfx_storage_t*)pfx);
  /* This superprefix might be a subprefix for some other superprefix */
  if(j!=kh_end(state->subprefix_map) && kh_value(state->subprefix_map,j).number_of_subasns>0){
    is_subprefix=1;
  }
  if (is_subprefix){
    ...
    subprefixes_in_superprefix_t* super2_khash=kh_value(state->superprefix_map,p);
    for (n = kh_begin(super1_khash); n != kh_end(super1_khash); ++n) {
      if(kh_exist(super1_khash,n)){
        kh_put(subprefixes_in_superprefix,super2_khash,kh_key(super1_khash,n),&ret);
        m=kh_get(subprefix_map,state->subprefix_map,kh_key(super1_khash,n));
        ...
        if(wandio_printf(state->file,"%"PRIu32"|%s|%s|FINISHED|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s    \n",
                                                   ...
                                                   submoas_struct.start,
                                                   timenow,
                                                   print_submoas_info(6,consumer,pfx, (bgpstream_pfx_t*)&submoas_struct.subprefix, asn_buffer, MAX_BUFFER_LEN)) == -1)
                                   {
                                     fprintf(stderr, "ERROR: Could not write %s file\n",state->filename);
                                     return ;
                                   }
    ...
    for (n = kh_begin(super1_khash); n != kh_end(super1_khash); ++n) {
      if(kh_exist(super1_khash,n)){
        ...
        if(wandio_printf(state->file,"%"PRIu32"|%s|%s|FINISHED|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s    \n",
                                                   ...
                                                   submoas_struct.start,
                                                   timenow,
                                                   print_submoas_info(8,consumer,pfx, (bgpstream_pfx_t*)&submoas_struct.subprefix, asn_buffer, MAX_BUFFER_LEN)) == -1)
                                   {
                                     fprintf(stderr, "ERROR: Could not write %s file\n",state->filename);
                                     return ;
                                   }
...
static void rem_patricia(bgpstream_patricia_tree_t *pt, bgpstream_patricia_node_t *node, void *data){
  ...
  int num_orig_asns=this_prefix_info->number_of_asns;
   for (j=0;j<num_orig_asns;j++){
      if(this_prefix_info->origin_asns[j].last_seen +state->window_size < time_now){
       ...
       /* Origin ASN is removed, check whether its prefix belongs to some prefix involved in a submoas */
        ...
        this_prefix_info->origin_asns[j]=this_prefix_info->origin_asns[num_orig_asns-1];
        num_orig_asns--;
        j--;
        }
...
int bvc_submoas_process_view(bvc_t *consumer, uint8_t interests, bgpview_t *view)
{
  ...
  state->file = NULL;
  if((state->file = wandio_wcreate(state->filename, wandio_detect_compression_type(state->filename), DEFAULT_COMPRESS_LEVEL, O_CREAT)) == NULL)
  {
  ...
  for(bgpview_iter_first_pfx(it, 0 /* all versions */, BGPVIEW_FIELD_ACTIVE);
    ...
    if (atleast_one==1 ){
      ...
      if(new_prefix){
        if ((pfx_node=bgpstream_patricia_tree_insert(state->pt, pfx)) ==NULL){
  ...
  snprintf(state->filename, MAX_BUFFER_LEN, OUTPUT_FILE_FORMAT".done",
           state->output_folder, state->time_now, state->current_window_size);
  if((f = wandio_wcreate(state->filename, wandio_detect_compression_type(state->filename), DEFAULT_COMPRESS_LEVEL, O_CREAT)) == NULL)
    {
      fprintf(stderr, "ERROR: Could not open %s for writing\n",state->filename);
      return -1;
    }
