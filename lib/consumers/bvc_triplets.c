...
#define STATE					\
  (BVC_GET_STATE(consumer, triplets))
...
KHASH_INIT(triplets_map, char*, triplet_info_t, 1, kh_str_hash_func, kh_str_hash_equal);
...
static int output_timeseries(bvc_t *consumer, uint32_t ts)
{
  ...
  timeseries_kp_set(state->kp, state->new_triplets_count_idx, state->new_triplets_count);
  timeseries_kp_set(state->kp, state->ongoing_triplets_count_idx, state->ongoing_triplets_count);
  timeseries_kp_set(state->kp, state->finished_triplets_count_idx, state->finished_triplets_count);
  timeseries_kp_set(state->kp, state->newrec_triplets_count_idx, state->newrec_triplets_count);
...
static int create_ts_metrics(bvc_t *consumer)
{
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "new_triplets_count");
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "finished_triplets_count");
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "newrec_triplets_count");
  ...
  snprintf(buffer, MAX_BUFFER_LEN, METRIC_PREFIX_FORMAT,
           CHAIN_STATE->metric_prefix, state->window_size, "ongoing_triplets_count");
...
void bvc_triplets_destroy(bvc_t *consumer)
{
  ...
  if(state!=NULL){
    bvc_triplets_state_t *state = STATE;
    khint_t k;
    for(k = kh_begin(state->triplets_map); k!= kh_end(state->triplets_map); k++){
...
void print_to_file_triplets(bvc_t *consumer, int status, char *triplet, triplet_info_t triplet_info,bgpstream_pfx_t *pfx){
  bvc_triplets_state_t *state = STATE;
  char pfx_str[MAX_BUFFER_LEN];

  if (status==NEW){
    //printf("printing \n");
    if(wandio_printf(state->file_triplets,"%"PRIu32"|%s|NEW|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s\n",
                  state->time_now,
                  triplet,triplet_info.first_seen,triplet_info.start,triplet_info.end,
  ...
  if (status==NEWREC){
    if(wandio_printf(state->file_triplets,"%"PRIu32"|%s|NEWREC|%"PRIu32"|%"PRIu32"|%"PRIu32"|%s\n",
                  state->time_now,
                  triplet,triplet_info.first_seen,triplet_info.start,triplet_info.end,
                  bgpstream_pfx_snprintf(pfx_str, INET6_ADDRSTRLEN+3, pfx)) == -1)
    {
  ...
  if (status==FINISHED){
    if(wandio_printf(state->file_triplets,"%"PRIu32"|%s|FINISHED|%"PRIu32"|%"PRIu32"|%"PRIu32"\n",
                  state->time_now,
                  triplet,triplet_info.first_seen,triplet_info.start,triplet_info.end
                  ) == -1)
    {
  ...
  for(k = kh_begin(state->triplets_map); k!= kh_end(state->triplets_map); k++){
    if(kh_exist(state->triplets_map,k)){
      ...
      if(triplet_info.ongoing){
        if(wandio_printf(state->file_triplets,"%"PRIu32"|%s|ONGOING|%"PRIu32"|%"PRIu32"|%"PRIu32"\n",
                      state->time_now,
                      kh_key(state->triplets_map,k),triplet_info.first_seen,triplet_info.start,triplet_info.end
                      ) == -1)
        {
         fprintf(stderr, "ERROR: Could not write %s file\n",state->filename_triplets);
         return ;
  ...
  for(k = kh_begin(state->triplets_map); k!= kh_end(state->triplets_map); k++){
    if(kh_exist(state->triplets_map,k)){
      ...
      if(triplet_info.last_seen  < state->time_now && triplet_info.ongoing){
        ...
        print_to_file_triplets(consumer,FINISHED,kh_key(state->triplets_map,k),triplet_info,NULL);
...
void insert_update_triplet(bvc_t *consumer,char *triplet,bgpstream_pfx_t *pfx)
{
  ...
  if(k==kh_end(state->triplets_map)){
    ...
    if(triplet_info.ongoing==0){
      triplet_info.ongoing=1;
      //Checking when was the last time it started. Make it newrec if last end is within the current window
      if(triplet_info.start+state->window_size > state->time_now){
          print_to_file_triplets(consumer,NEWREC,kh_key(state->triplets_map,k),triplet_info,pfx);
          state->newrec_triplets_count++;
      }
      else{
          print_to_file_triplets(consumer,NEW,kh_key(state->triplets_map,k),triplet_info,pfx);
          state->new_triplets_count++;
      }
      triplet_info.start=state->time_now;
    }
...
int bvc_triplets_process_view(bvc_t *consumer, uint8_t interests, bgpview_t *view)
{
  ...
  snprintf(state->filename_triplets, MAX_BUFFER_LEN, OUTPUT_FILE_FORMAT_TRIPLETS,
           state->output_folder, time_now, state->window_size);
    /* open file for writing */
  if((state->file_triplets = wandio_wcreate(state->filename_triplets, wandio_detect_compression_type(state->filename_triplets), DEFAULT_COMPRESS_LEVEL, O_CREAT)) == NULL)
    {
                ...
                {         
                  ...
                  if(normal_asn){
                    ...
                    if(prev_prev_asn!=0){
                      ...
                      ret =snprintf(triplet, buffer_len - 1, "%"PRIu32"-%"PRIu32"-%"PRIu32" ",prev_prev_asn, prev_asn, asn);  
  ...
  snprintf(filename, MAX_BUFFER_LEN, OUTPUT_FILE_FORMAT_TRIPLETS".done",
           state->output_folder, time_now, state->window_size);
  if((done_triplets = wandio_wcreate(filename, wandio_detect_compression_type(filename), DEFAULT_COMPRESS_LEVEL, O_CREAT)) == NULL)
    {
      fprintf(stderr, "ERROR: Could not open %s for writing\n",filename);
      return -1;
    }
