    ...
    if(kh_exist(state->edges_map,k)){
      ...
      for(j = kh_begin(edge_list); j!= kh_end(edge_list); j++){
        if(kh_exist(edge_list,j)){
          ...
          if(edge_info.last_seen + state->window_size < state->time_now && edge_info.ongoing){
...
int insert_update_edges(bvc_t *consumer, uint32_t asn1, uint32_t asn2, bgpstream_pfx_t *pfx){
  int ret, category;
  category=-1;
  bvc_edges_state_t *state = STATE;
  khint_t k,j,p;
